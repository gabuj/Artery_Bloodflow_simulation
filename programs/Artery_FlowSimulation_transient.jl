...
U = TransientTrialFESpace(V,[uDwalls])
...
p_inlet_func(t) = 1000.0 * sin(0.5*pi*t)   # your original
...
# Residual and jac for this time-step (closures capture pval)
res_t(t,(u,p),(v,q)) = a((u,p),(v,q)) + c(u,v) - neumann_t(t,(u,v))

jac_t((u,p),(du,dp),(v,q)) = a((du,dp),(v,q)) + dc(u,du,v) # + terms from dneumann if linearized???
...
#(we could have a runge kutta solver instead)
solver = ThetaMethod(nls, Δt, θ)


##############
#solve the problem
# Build FEFunctions for initial guess:
zero_velocity(x) = VectorValue(0.0,0.0,0.0)
zero_pressure(x) = 0.0

uh_prev = interpolate(zero_velocity, U)   # velocity FEFunction in U
ph_prev = interpolate(zero_pressure, P)   # pressure FEFunction in P

uh0 = interpolate_everywhere(g(t0), X(t0))

uh, ph = solve(solver, op_sl, t0, tF, uh0)



#save the solution
outputfile = "first_steps/tutorial_outputs/transient/nonlinear/transient_artery"

createpvd(outputfile) do pvd
  pvd[0] = createvtk(Ω, outputfile * "_0" * ".vtu", cellfields=["uh"=>uh,"ph"=>ph])
  for (tn, uhn) in uh
    pvd[tn] = createvtk(Ω, outputfile * "_$tn" * ".vtu", cellfields=["uh"=>uh,"ph"=>ph])
  end
end
